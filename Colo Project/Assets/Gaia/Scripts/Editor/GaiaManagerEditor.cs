...
namespace Gaia
{
    ...
    public class GaiaManagerEditor : EditorWindow
    {
        ...
        public static GaiaResource CreateResourcesAsset()
        {
            ...
            #else
            string[] path = EditorSceneManager.GetActiveScene().name.Split(char.Parse("/"));
            #endif
            if (path.Length > 0)
            {
                ...
                {
                    AssetDatabase.CreateAsset(resources, string.Format("Assets/Gaia/Data/GR-{0}-{1:yyyyMMdd-HHmmss}.asset", sceneName, DateTime.Now));
                }
                else
                {
        ...
        public static void SetGaiaDefines()
        {
            ...
            //Check for ImageFX and inject GAIA_IMAGEFX_PRESENT
            if (GotImageFX() == true)
            {
                if (!currBuildSettings.Contains("GAIA_IMAGEFX_PRESENT"))
                {
                    PlayerSettings.SetScriptingDefineSymbolsForGroup(EditorUserBuildSettings.selectedBuildTargetGroup, currBuildSettings + ";GAIA_IMAGEFX_PRESENT");
                }
            }
            else
            {
                //Remove it if its not there
                if (currBuildSettings.Contains("GAIA_IMAGEFX_PRESENT"))
                {
                    currBuildSettings = currBuildSettings.Replace("GAIA_IMAGEFX_PRESENT", "");
